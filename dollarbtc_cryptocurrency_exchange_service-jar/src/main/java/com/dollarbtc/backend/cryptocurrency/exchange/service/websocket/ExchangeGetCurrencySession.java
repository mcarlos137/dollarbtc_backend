 ...
 * @author CarlosDaniel
 */
public class ExchangeGetCurrencySession {
        ...
        public void run() {
            try {
                for(Session session : sessions.keySet()){
                    ...
                    session.getRemote().sendString(ServiceUtil.createWSResponse(ExchangeOperation.getCurrency(currency), null, "result").toString());
                    removeSession(session, 100, "normal close");
                }
            } catch (IOException ex) {
                Logger.getLogger(ExchangeGetCurrencySession.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
