 ...
 * @author CarlosDaniel
 */
public class ExchangeGetSymbolsSession {
        ...
        public void run() {
            try {
                for (Session session : sessions.keySet()) {
                    session.getRemote().sendString(ServiceUtil.createWSResponse(ExchangeOperation.getSymbols(), null, "result").toString());
                    removeSession(session, 100, "normal close");
                }
            } catch (IOException ex) {
                Logger.getLogger(ExchangeGetSymbolsSession.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
