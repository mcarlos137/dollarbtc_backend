 ...
 * @author CarlosDaniel
 */
public class ExchangeGetSymbolSession {
        ...
        public void run() {
            try {
                for(Session session : sessions.keySet()){
                    ...
                    session.getRemote().sendString(ServiceUtil.createWSResponse(ExchangeOperation.getSymbol(symbol), null, "result").toString());
                    removeSession(session, 100, "normal close");
                }
            } catch (IOException ex) {
                Logger.getLogger(ExchangeGetSymbolSession.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
