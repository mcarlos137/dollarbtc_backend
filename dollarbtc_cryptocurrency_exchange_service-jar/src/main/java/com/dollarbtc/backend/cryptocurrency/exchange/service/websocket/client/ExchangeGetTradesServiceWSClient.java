 ...
 * @author CarlosDaniel
 */
public class ExchangeGetTradesServiceWSClient extends WebSocketClient {

    private final static String URI = "ws://localhost:8081/exchange";

    public ExchangeGetTradesServiceWSClient() {
        super(URI);
        prepareRequest();
    }
    ...
    protected void prepareRequest() {
        ...
        ((ObjectNode) jsonNode).put("method", "getTrades");
        JsonNode params = mapper.createObjectNode();
        ((ObjectNode) params).put("symbol", "BTCUSDT");
        ((ObjectNode) params).put("limit", 2);
        ((ObjectNode) params).put("sort", "DESC");
        ((ObjectNode) params).put("by", "id");
    ...
    private void keepAlive() {
        while (true) {
            try {
                ...
                Logger.getLogger(ExchangeGetTradesServiceWSClient.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
