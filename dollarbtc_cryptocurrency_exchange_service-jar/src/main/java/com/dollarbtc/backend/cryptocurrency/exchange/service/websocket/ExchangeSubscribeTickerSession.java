 ...
 * @author CarlosDaniel
 */
public class ExchangeSubscribeTickerSession {
        ...
        public void run() {
            try {
                for (Session session : sessions.keySet()) {
                    ...
                    Object[] tickerObject = ExchangeOperation.subscribeTicker(symbol, sessionsLastId.get(session));
                    if (tickerObject == null) {
                        continue;
                    } 
                    session.getRemote().sendString(ServiceUtil.createWSResponse((JsonNode) tickerObject[1], "ticker", "params").toString());
                    sessionsLastId.put(session, (Long) tickerObject[0]);
                }
