 ...
 * @author carlosmolina
 */
public class CryptoAPIsCheckInEthereumTokenTransactions extends AbstractOperation<Void> {

    private final String userName, currency;

    public CryptoAPIsCheckInEthereumTokenTransactions(String userName, String currency) {
        super(Void.class);
    ...
    protected void execute() {
        Logger.getLogger(CryptoAPIsCheckInEthereumTokenTransactions.class.getName()).log(Level.INFO, userName);
        String blockchain = "ethereum";
        Integer confirms = 20;
        String walletsTag = "mcWalletsEthereum";
        ...
        if (addressesCurrencyTransactionsTypeDataFile.isFile()) {
            try {
                ...
                Logger.getLogger(CryptoAPIsCheckInEthereumTokenTransactions.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            updateBlockchainHeight = true;
        }
        if (updateBlockchainHeight) {
            ...
            if (blockchainHeight == null) {
                Logger.getLogger(CryptoAPIsCheckInEthereumTokenTransactions.class.getName()).log(Level.SEVERE, "BLOCKCHAIN HEIGHT IS NULL");
                return;
            }
        ...
        for (String walletStatusTag : walletStatusTagRequestTime.keySet()) {
            if (!addressesCurrencyTransactionsTypeUserControlFile.isFile()) {
                ...
                try {
                    ...
                    if (addressesCurrencyTransactionsTypeUserControl.has("lastRequestTimestamp") && 
                            DateUtil.parseDate(DateUtil.getDateMinutesBefore(currentTimestamp, walletStatusTagRequestTime.get(walletStatusTag))).before(DateUtil.parseDate(addressesCurrencyTransactionsTypeUserControl.get("lastRequestTimestamp").textValue()))) {
                        return;
                    } else {
                        ((ObjectNode) addressesCurrencyTransactionsTypeUserControl).put("lastRequestTimestamp", currentTimestamp);
            ...
            JsonNode transactions = new CryptoAPIsGetEthereumTokenTransactions(addresses, "IN", "mainnet", currency, blockchainHeight).getResponse();
