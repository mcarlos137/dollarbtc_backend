 ...
 * @author carlosmolina
 */
public class MCUserGetMessages extends AbstractOperation<JsonNode> {

    private final String userName, chatRoom;
    private final boolean old;

    public MCUserGetMessages(String userName, String chatRoom, boolean old) {
...
//                    }
                    if (!old) {
                        ...
                        if (!userName.equals(senderUserName)) {
                            ...
                            FileUtil.createFile(mcUserMessage, new File(UsersFolderLocator.getMCMessagesFolder(senderUserName, userName), mcUserMessage.get("id").textValue() + ".json"));
                        }
                        FileUtil.moveFileToFolder(orderedFiles.get(key), FileUtil.createFolderIfNoExist(new File(mcUserMessagesFolder, "Old")));
                    }
                } catch (IOException ex) {
                    Logger.getLogger(MCUserGetMessages.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            if(mcUserMessagesArray.size() > 0){
