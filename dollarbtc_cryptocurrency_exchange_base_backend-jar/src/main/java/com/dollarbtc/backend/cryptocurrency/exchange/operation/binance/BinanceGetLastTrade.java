 ...
 * @author carlosmolina
 */
public class BinanceGetLastTrade extends AbstractOperation<JsonNode> {

    private final String symbol;

    public BinanceGetLastTrade(String symbol) {
    ...
    public void execute() {
        ...
        for(File tradeFile : ExchangesFolderLocator.getExchangeSymbolTradesFolder("Binance", symbol).listFiles()){
            if(!tradeFile.isFile()){
                continue;
            }
        ...
        try {
            ...
            Logger.getLogger(BinanceGetLastTrade.class.getName()).log(Level.SEVERE, null, ex);
        }
        super.response = mapper.createObjectNode();
    }
