 ...
 * @author carlosmolina
 */
public class CryptoAPIsCheckInCoinTransaction extends AbstractOperation<Boolean> {

    private final String userName, address, transactionId, currency;

    public CryptoAPIsCheckInCoinTransaction(String userName, String address, String transactionId, String currency) {
        super(Boolean.class);
    ...
    protected void execute() {
        Logger.getLogger(CryptoAPIsCheckInCoinTransaction.class.getName()).log(Level.INFO, address);
        super.response = true;
        ...
        if (!addressesCurrencyTransactionsTypeUserControlFile.isFile()) {
            ...
            try {
                ...
                Logger.getLogger(CryptoAPIsCheckInCoinTransaction.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        JsonNode transaction = new CryptoAPIsGetCoinTransansaction("IN", blockchain, "mainnet", transactionId, address).getResponse();
