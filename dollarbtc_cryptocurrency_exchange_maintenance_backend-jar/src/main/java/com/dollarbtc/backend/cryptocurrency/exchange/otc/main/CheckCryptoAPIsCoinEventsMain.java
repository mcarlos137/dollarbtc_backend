...
import com.dollarbtc.backend.cryptocurrency.exchange.operation.cryptoapis.CryptoAPIsCheckInCoinTransaction;
 ...
 * @author CarlosDaniel
 */
public class CheckCryptoAPIsCoinEventsMain {

    public static void main(String[] args) {
        ObjectMapper mapper = new ObjectMapper();
        Map<String, String> currencyBlockchain = new HashMap<>();
        currencyBlockchain.put("BTC", "bitcoin");
        currencyBlockchain.put("ETH", "ethereum");
        for (String currency : currencyBlockchain.keySet()) {
            String blockchain = currencyBlockchain.get(currency);
            File cryptoAPIsEventsFolder = CryptoAPIsFolderLocator.getEventsFolder(blockchain, "mainnet", "ADDRESS_COINS_TRANSACTION_UNCONFIRMED");
            File cryptoAPIsEventsOldFolder = CryptoAPIsFolderLocator.getEventsOldFolder(blockchain, "mainnet", "ADDRESS_COINS_TRANSACTION_UNCONFIRMED");
            for (File cryptoAPIsEventsFile : cryptoAPIsEventsFolder.listFiles()) {
                ...
                try {
                    ...
                    Logger.getLogger(CheckCryptoAPIsCoinEventsMain.class.getName()).log(Level.INFO, "userName {0}", userName);
                    System.out.println("transactionId " + transactionId);
                    System.out.println("address " + address);
                    Boolean allConfirmed = new CryptoAPIsCheckInCoinTransaction(userName, address, transactionId, currency).getResponse();
                    if (allConfirmed) {
                    ...
                    try {
                        ...
                        Logger.getLogger(CheckCryptoAPIsCoinEventsMain.class.getName()).log(Level.SEVERE, null, ex);
                    }
